...
export function LineAreaChart({
  ...
  const areaPath = `M ${getX(0)},${padding.top + chartH} L ${points} L ${getX(
    values.length - 1
  ...
  const gradId = `areaGrad_${color}_${Math.random().toString(36).substr(2, 6)}`;
    ...
    <div className="relative w-full overflow-hidden">
      ...
      >
          ...
          return (
            <g key={i}>
              ...
              <circle
                ...
                stroke={isHovered ? strokeColor : "#06080f"}
                strokeWidth="2"
                className="transition-all duration-200 pointer-events-none"
              />
              {/* X label */}
              {labels[i] && (i % Math.ceil(labels.length / 8) === 0 || i === labels.length - 1) && (
                <text
                  x={cx}
        ...
        {hoverIndex !== null && values[hoverIndex] !== undefined && (
          <g transform={`translate(${getX(hoverIndex)}, ${getY(values[hoverIndex])})`}>
            <line
              x1="0"
              y1={-getY(values[hoverIndex]) + padding.top}
              x2="0"
              y2={padding.top + chartH - getY(values[hoverIndex])}
